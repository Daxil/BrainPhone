...
const API_BASE_URL = 'https://bba8vah5ofa4lbqtm3sb.containers.yandexcloud.net';
            ...
            return {
              ...
              audioRecordings: p.audio_files?.map(function(af: any) {
                return {
                  ...
                  url: af.yandex_disk_url || (API_BASE_URL + af.file_path),
    ...
    if (result.success) {
      const audioUrl = result.data?.data?.audio?.yandex_disk_url
        ...
        : (result.data?.data?.audio?.file_path
            ? (API_BASE_URL + result.data.data.audio.file_path)
            : data.url);
