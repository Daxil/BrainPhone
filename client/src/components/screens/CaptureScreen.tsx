...
import { Mic, Play, Pause, Upload, X, AlertCircle, CheckCircle, RotateCcw, Headphones } from 'lucide-react';
...
export default function CaptureScreen({
  ...
  const handleReRecord = (id: string) => {
    onUpdateRecording(id, { status: 'pending' });
    onSelectRecording(id);
    onReRecord();
  };

  const handleSaveCurrentRecording = async () => {
    const saveId = pendingSaveId || currentRecordingId;
    if (audioData?.blob && !isRecording && saveId) {
      setIsSaving(true);
      try {
        await onSaveRecording(saveId, audioData);
      } catch (err) {
  ...
  return (
    ...
    {/* Блок управления после остановки записи */}
    {audioData && !isRecording && (pendingSaveId || currentRecordingId) && (
      ...
      <button onClick={() => handleReRecord(pendingSaveId || currentRecordingId!)} className="bg-gray-500 hover:bg-gray-600 text-white px-6 py-4 rounded-full flex items-center gap-2">
      ...
      {audioData && (() => {
        const saveRec = mergedRecordings.find(r => r.id === (pendingSaveId || currentRecordingId));
        if (!saveRec) return null;
        const status = validateDuration(audioData.duration, saveRec.minDuration, saveRec.maxDuration);
        return (
          <div className={`mt-4 flex justify-center gap-2 ${status === 'warning' ? 'text-yellow-600' : 'text-green-600'}`}>
          {status === 'warning' ? <AlertCircle className="w-5 h-5" /> : <CheckCircle className="w-5 h-5" />}
          <span>{status === 'warning' ? 'Длительность не соответствует' : 'Длительность в норме'}</span>
          </div>
        );
      })()}
      </div>
    ...
    {/* Блок управления для УЖЕ СОХРАНЕННОЙ записи (зеленая галочка) */}
    {currentRecording?.status === 'completed' && !audioData && !isRecording && (
      <div className="border-t pt-6">
      <p className="text-center text-gray-700 font-medium mb-4">Запись сохранена:</p>
      <div className="flex justify-center gap-4 flex-wrap">
      <button onClick={() => onPlayAudio(currentRecording.url)} className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-4 rounded-full flex items-center gap-2">
      <Headphones className="w-6 h-6" /> Прослушать
      </button>
      <button onClick={() => handleReRecord(currentRecording.id)} className="bg-gray-500 hover:bg-gray-600 text-white px-6 py-4 rounded-full flex items-center gap-2">
      <RotateCcw className="w-6 h-6" /> Перезаписать
      </button>
      </div>
      </div>
